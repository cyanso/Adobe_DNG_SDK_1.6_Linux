 ...
 *
 * All conditional compilation macros for the DNG SDK begin with a lowercase
 * 'q'.
 */
...
#if !(defined(qMacOS) || defined(qWinOS) || defined(qAndroid) ||               \
      defined(qiPhone) || defined(qLinux) || defined(qWeb))
#include "RawEnvironment.h"
#endif

// This requires a force include or compiler define.  These are the unique
// platforms.
...
// Platforms.
// Zeros out any undefined platforms, so that #if can be used in place of
// #ifdef.
...
/// \def qDNGDebug
/// 1 if debug code is compiled in, 0 otherwise. Enables assertions and other
...
/// \def qDNGThreadSafe
/// 1 if target platform has thread support and threadsafe libraries, 0
/// otherwise.
...
/// 1 if dng_show_message should use fprintf to stderr. 0 if it should use a
/// platform specific interrupt mechanism.
...
// - maximum total number of pixels is 10 gigapixels (10 * 1000 * 1000 * 1000
// pixels)
//
// When set to false:
