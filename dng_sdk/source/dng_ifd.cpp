...
bool
dng_ifd::ParseTag(dng_host& host,
                  dng_stream& stream,
                  uint32 parentCode,
                  uint32 tagCode,
                  uint32 tagType,
                  ...
                  uint64 tagOffset)
{
  ...
  switch (tagCode) {
    ...
    case tcSubIFDs: {
      ...
      CheckTagType(
        parentCode, tagCode, tagType, ttLong, ttIFD, ttLong8, ttIFD8);
      ...
      if (gVerbose) {
        ...
        DumpTagValues(stream, "IFD", parentCode, tagCode, tagType, tagCount);
      }
    ...
    case tcCFAPattern: {
      ...
      if (!CheckTagCount(parentCode,
                         tagCode,
                         tagCount,
                         fCFARepeatPatternRows * fCFARepeatPatternCols)) {
        return false;
      }
    ...
    case tcBlackLevel: {
      ...
      if (!CheckTagCount(parentCode,
                         tagCode,
                         tagCount,
                         fBlackLevelRepeatRows * fBlackLevelRepeatCols *
                           fSamplesPerPixel)) {
        return false;
      }
      ...
      if (fBlackLevelRepeatRows < 1 ||
          ...
          fBlackLevelRepeatCols < 1 ||
          fBlackLevelRepeatCols > kMaxBlackPattern || fSamplesPerPixel < 1 ||
          fSamplesPerPixel > kMaxColorPlanes) {
        return false;
      }
    ...
    case tcPreviewColorSpace: {
      ...
      fPreviewInfo.fColorSpace =
        (PreviewColorSpaceEnum)stream.TagValue_uint32(tagType);
    ...
    case tcPreviewDateTime: {
      ...
      ParseStringTag(
        stream, parentCode, tagCode, tagCount, fPreviewInfo.fDateTime, false);
    ...
    case tcNoiseProfile: {
      ...
      if (gVerbose) {
        ...
        for (uint32 plane = 0; plane < numPlanes; plane++) {
          ...
          printf(
            "  Noise function for plane %u: scale = %.20lf, offset = %.20lf\n",
            (unsigned)plane,
    ...
    case tcEnhanceParams: {
      ...
      ParseStringTag(
        stream, parentCode, tagCode, tagCount, fEnhanceParams, false);
    ...
    case tcProfileGainTableMap: {
      ...
      fProfileGainTableMap.reset(dng_gain_table_map::GetStream(host, stream));
    ...
    case tcRGBTables: {
      ...
      fMaskedRGBTables.reset(
        dng_masked_rgb_tables::GetStream(host, stream, isDraft));
    ...
    case tcSemanticName: {
      ...
      ParseStringTag(
        stream, parentCode, tagCode, tagCount, fSemanticName, false);
    ...
    case tcSemanticInstanceID: {
      ...
      ParseStringTag(
        stream, parentCode, tagCode, tagCount, fSemanticInstanceID, false);
...
bool
dng_ifd::IsValidCFA(dng_shared& shared, uint32 parentCode)
{
  ...
  for (j = 0; j < fCFARepeatPatternRows; j++) {
    ...
    for (k = 0; k < fCFARepeatPatternCols; k++) {
      ...
      if (!found) {
        ...
        ReportError(
          "CFAPattern contains colors not included in the CFAPlaneColor tag",
          LookupParentCode(parentCode));
  ...
  for (n = 0; n < shared.fCameraProfile.fColorPlanes; n++) {
    ...
    if (count[n] == 0) {
      ...
      ReportError(
        "CFAPattern does not contain all the colors in the CFAPlaneColor tag",
        LookupParentCode(parentCode));
...
bool
dng_ifd::IsValidDNG(dng_shared& shared, uint32 parentCode)
{
  ...
  uint32 defaultWhite =
    isFloatingPoint ? 1 : (uint32)((((uint64)1) << fBitsPerSample[0]) - 1);
  ...
  if (fNewSubFileType != sfMainImage && fNewSubFileType != sfPreviewImage &&
      fNewSubFileType != sfTransparencyMask &&
      fNewSubFileType != sfPreviewMask && fNewSubFileType != sfDepthMap &&
  ...
  if (fNewSubFileType == sfTransparencyMask ||
    ...
    if (fPhotometricInterpretation != piTransparencyMask) {
      ...
      ReportError(
        "NewSubFileType requires PhotometricInterpretation = TransparencyMask",
        LookupParentCode(parentCode));
    ...
    if (fPhotometricInterpretation != piPhotometricMask) {
      ...
      ReportError(
        "NewSubFileType requires PhotometricInterpretation = PhotometricMask",
        LookupParentCode(parentCode));
  ...
  switch (fPhotometricInterpretation) {
    ...
    case piCFA: {
      ...
      if (isMonochrome) {
        ...
        ReportError(
          "PhotometricInterpretation requires use of ColorMatrix1 tag",
          LookupParentCode(parentCode));
  ...
  if (isFloatingPoint) {
    ...
    if (fPhotometricInterpretation != piCFA &&
      ...
      ReportError("Floating point data requires PhotometricInterpretation CFA "
                  "or LinearRaw or TransparencyMask",
                  LookupParentCode(parentCode));
  ...
  for (j = 0; j < kMaxSamplesPerPixel; j++) {
    ...
    if (j < fSamplesPerPixel) {
      ...
      if (isFloatingPoint && fBitsPerSample[j] != 16 &&
          fBitsPerSample[j] != 24 && fBitsPerSample[j] != 32) {
      ...
      if (minBitsPerSample == 8 && maxBitsPerSample == 16 &&
          fBitsPerSample[j] != 8 && fBitsPerSample[j] != 16) {

#if qDNGValidate

        ReportError(
          "Rendered previews and integer masks require 8 or 16 bits per sample",
          LookupParentCode(parentCode));
  ...
  switch (fCompression) {
    ...
    case ccJPEG: {
      ...
      if (fPhotometricInterpretation == piRGB) {
        ...
        ReportError(
          "JPEG previews should use PhotometricInterpretation = YCbYb",
          LookupParentCode(parentCode));
    ...
    case ccLossyJPEG: {
      ...
      if (fPhotometricInterpretation != piLinearRaw
        ...
        ReportError("Lossy JPEG compression code requires "
                    "PhotometricInterpretation = LinearRaw or PhotometricMask",
                    LookupParentCode(parentCode));
    ...
    case ccDeflate: {
      ...
      if (!isFloatingPoint && fBitsPerSample[0] != 32 &&
          fPhotometricInterpretation != piTransparencyMask &&
#if qDNGSemanticMask
          fPhotometricInterpretation != piPhotometricMask &&
#endif
  ...
  if (isFloatingPoint && fCompression == ccDeflate &&
      (fPredictor == cpFloatingPoint || fPredictor == cpFloatingPointX2 ||
  ...
  for (j = 0; j < fSamplesPerPixel; j++) {
    ...
    if (fSampleFormat[j] !=
        (isFloatingPoint ? sfFloatingPoint : sfUnsignedInteger)) {
  ...
  for (j = 0; j < fSamplesPerPixel; j++) {
    ...
    if (fWhiteLevel[j] < 1.0 ||
        (fWhiteLevel[j] > maxWhite && !isFloatingPoint)) {

#if qDNGValidate
      std::cout << fWhiteLevel[j] << " " << maxWhite << " " << isFloatingPoint
                << std::endl;
  ...
  // Check DefaultScaleH.	 Enhanced IFDs default to invalid, so allow
  // that.
  ...
  // Check DefaultScaleV.	 Enhanced IFDs default to invalid, so allow
  // that.
  ...
  if (fNewSubFileType != sfEnhancedImage || fDefaultCropSizeH.IsValid() ||
  ...
  if (fDefaultCropOriginH.As_real64() + fDefaultCropSizeH.As_real64() >
        (real64)fActiveArea.W() ||
      fDefaultCropOriginV.As_real64() + fDefaultCropSizeV.As_real64() >
        (real64)fActiveArea.H()) {
  ...
  if (fPhotometricInterpretation == piCFA) {
    ...
    if (fDefaultCropOriginH.As_real64() + fDefaultCropSizeH.As_real64() >
        (real64)fActiveArea.W() - kMinPad) {

      ReportWarning("Too little padding on right edge of CFA image",
                    "possible interpolation artifacts");
    }

    if (fDefaultCropOriginV.As_real64() + fDefaultCropSizeV.As_real64() >
        (real64)fActiveArea.H() - kMinPad) {
  ...
  if (fRowInterleaveFactor != 1) {
    ...
    if (shared.fDNGBackwardVersion < dngVersion_1_2_0_0) {
      ...
      ReportError(
        "Non-default RowInterleaveFactor tag not allowed in this DNG version",
        LookupParentCode(parentCode));
  ...
  if (fSubTileBlockRows != 1 || fSubTileBlockCols != 1) {
    ...
    if (shared.fDNGBackwardVersion < dngVersion_1_2_0_0) {
      ...
      ReportError(
        "Non-default SubTileBlockSize tag not allowed in this DNG version",
        LookupParentCode(parentCode));
...
dng_rect
dng_ifd::TileArea(uint32 rowIndex, uint32 colIndex) const
{
...
uint32
dng_ifd::TileByteCount(const dng_rect& tile) const
{
...
uint64
dng_ifd::MaxImageDataByteCount() const
{

  uint64 bitsPerRow =
    (uint64)fTileWidth * (uint64)fSamplesPerPixel * (uint64)fBitsPerSample[0];
...
void
dng_ifd::FindTileSize(uint32 bytesPerTile, uint32 cellH, uint32 cellV)
{
  ...
  DNG_REQUIRE(across > 0,
              "Bad number of tiles across in dng_ifd::FindTileSize");
...
void
dng_ifd::FindStripSize(uint32 bytesPerStrip, uint32 cellV)
{
...
void
dng_ifd::ReadImage(dng_host& host,
                   dng_stream& stream,
                   dng_image& image,
